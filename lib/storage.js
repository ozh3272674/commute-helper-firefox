...
const STORAGE_KEYS = {
  ...
  WEATHER_CITY: 'weather_city',        // v2.5: 天气查询城市 { name, adcode }
  LOCATION_PRESETS: 'location_presets', // v2.6: 地点预设库 [{ id, name, origin, destination }]
  API_COUNT: 'api_call_count',          // v2.6: API 调用计数 { date, count }
  API_LIMIT: 'api_daily_limit',        // v2.6: 每日配额上限，默认5000
};
...
export async function exportData() {
  ...
  const filename = `通勤助手_备份_${timestamp}.json`;

  // 发送到后台脚本处理下载（支持 saveAs 对话框）
...
export async function getWeatherCity() {
  const result = await browser.storage.local.get(STORAGE_KEYS.WEATHER_CITY);
  return result[STORAGE_KEYS.WEATHER_CITY] || null; // { name: '苏州市', adcode: '320500' }
}

export async function setWeatherCity(city) {
  await browser.storage.local.set({ [STORAGE_KEYS.WEATHER_CITY]: city });
}

// ==================== v2.6 地点预设库 ====================

export async function getLocationPresets() {
  const result = await browser.storage.local.get(STORAGE_KEYS.LOCATION_PRESETS);
  return result[STORAGE_KEYS.LOCATION_PRESETS] || [];
}

export async function saveLocationPreset(preset) {
  ...
  await browser.storage.local.set({ [STORAGE_KEYS.LOCATION_PRESETS]: presets });
  return preset;
}

export async function deleteLocationPreset(id) {
  const presets = (await getLocationPresets()).filter(p => p.id !== id);
  await browser.storage.local.set({ [STORAGE_KEYS.LOCATION_PRESETS]: presets });
}

// ==================== v2.6 API 调用计数 ====================

export async function incrementApiCount(count = 1) {
  const today = formatDateStr(new Date());
  const result = await browser.storage.local.get(STORAGE_KEYS.API_COUNT);
  const stored = result[STORAGE_KEYS.API_COUNT] || { date: today, count: 0 };
  if (stored.date !== today) { stored.date = today; stored.count = 0; }
  stored.count += count;
  await browser.storage.local.set({ [STORAGE_KEYS.API_COUNT]: stored });
  return stored;
}
...
export async function getApiLimit() {
  const result = await browser.storage.local.get(STORAGE_KEYS.API_LIMIT);
  return result[STORAGE_KEYS.API_LIMIT] || 5000;
}

export async function setApiLimit(limit) {
  await browser.storage.local.set({ [STORAGE_KEYS.API_LIMIT]: limit });
}
